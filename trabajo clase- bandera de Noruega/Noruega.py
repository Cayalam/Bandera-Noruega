...
frame_entrada.config(bg="red", width=200, height=200)
frame_entrada.place(x=0, y=300)

#----------------------------------
frame_operacion = Frame(ventana_principal)
frame_operacion.config(bg="red", width=200, height=200)
frame_operacion.place(x=0, y=0)

#----------------------------------
frame_resultado = Frame(ventana_principal)
frame_resultado.config(bg="red", width=700, height=200)
...
#--------------------------------------------------------
frame_resultado = Frame(ventana_principal)
frame_resultado.config(bg="blue", width=50, height=500)
frame_resultado.place(x=225, y=0)

#----------------------------------
frame_entrada = Frame(ventana_principal)
frame_entrada.config(bg="blue", width=1000, height=50)
